...
class DataFile(AbstractLayout):
    ...
    def processVarBinds(self, varBinds, **context):
        ...
        for oid, val in varBinds:
            ...
            try:
                line = self._recordIndex.lookup(
                    str(univ.OctetString('.'.join(['%s' % x for x in oid]))))

            except KeyError:
                offset = searchRecordByOid(oid, text, self._textParser)
            ...
            while True:
                if exactMatch:
                    if context.get('nextFlag') and not subtreeFlag:
                        ...
                        if _nextLine:
                            _nextOid, _ = self._textParser.evaluate(
                                _nextLine, oidOnly=True)

                            try:
                                _, subtreeFlag, _ = self._recordIndex.lookup(
                    ...
                    if line:
                        _oid, _ = self._textParser.evaluate(
                            line, oidOnly=True
                        )
                    ...
                    try:
                        _, _, _prevOffset = self._recordIndex.lookup(
                        ...
                        if _prevOffset >= 0:
                            ...
                            _prevOid, _ = self._textParser.evaluate(
                ...
                try:
                    _oid, _val = self._textParser.evaluate(
                        line, **callContext)
    ...
    def __str__(self):
        return '%s controller' % self._textFile
...
class MibInstrumController:
    ...
    @staticmethod
    def _getCallContext(acInfo, nextFlag=False, setFlag=False):
        if acInfo is None:
    ...
    def readVars(self, varBinds, acInfo=None):
        return self.__dataFile.processVarBinds(
                varBinds, **self._getCallContext(acInfo, False))

    def readNextVars(self, varBinds, acInfo=None):
        return self.__dataFile.processVarBinds(
                varBinds, **self._getCallContext(acInfo, True))

    def writeVars(self, varBinds, acInfo=None):
        return self.__dataFile.processVarBinds(
                varBinds, **self._getCallContext(acInfo, False, True))
...
def main():
    ...
    Usage: %s [--help]
        ...
        [--agent-unix-endpoint=</path/to/named/pipe>]""" % (
            ...
            '|'.join([x for x in getattr(
                pysnmp_debug, 'FLAG_MAP', getattr(pysnmp_debug, 'flagMap', ()))
                      if x != 'mibview']),
            '|'.join([x for x in getattr(
                pyasn1_debug, 'FLAG_MAP', getattr(pyasn1_debug, 'flagMap', ()))]),
        ...
        for dataDir in dataDirs:
            ...
            for fullPath, textParser, communityName in getDataFiles(dataDir):
                if communityName in _dataFiles:
                    ...
                    dataFile = DataFile(
                        fullPath, textParser, variationModules).indexText(
                            forceIndexBuild, validateData)

                    mibInstrum = CONTROLLER_SET[dataFile.layout](dataFile)

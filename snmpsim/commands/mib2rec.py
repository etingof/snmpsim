...
def main():
    ...
    stringPool = ('Jaded zombies acted quaintly but kept driving their '
                  'oxen forward'.split())
...
""" % (os.path.basename(sys.argv[0]),
       '|'.join([x for x in getattr(debug, 'FLAG_MAP',
                                    getattr(debug, 'flagMap', ()))
        ...
        while True:
            if makeGuess:
                if isinstance(syntax, rfc1902.IpAddress):
                    ...
                    value = random.randrange(
                        timeticksRange[0], timeticksRange[1])

                elif isinstance(syntax, rfc1902.Gauge32):
                    value = random.randrange(
                        gaugeRange[0], gaugeRange[1])
                    ...
                    value = random.randrange(
                        int32Range[0], int32Range[1])

                elif isinstance(syntax, rfc1902.Unsigned32):
                    value = random.randrange(
                        unsignedRange[0], unsignedRange[1])

                elif isinstance(syntax, rfc1902.Counter64):
                    value = random.randrange(
                        counter64Range[0], counter64Range[1])

                elif isinstance(syntax, univ.OctetString):
                    maxWords = 10
                    value = ' '.join(
                        [stringPool[random.randrange(0, len(stringPool))]
            ...
            try:
                if syntax.tagSet == rfc1902.Integer32.tagSet:
                    return rfc1902.Integer32(syntax.clone(value))

                if syntax.tagSet == rfc1902.Unsigned32.tagSet:
                    return rfc1902.Unsigned32(syntax.clone(value))

                if syntax.tagSet == rfc1902.Bits.tagSet:
                    return rfc1902.OctetString(syntax.clone(value))
            ...
            sys.stderr.write('%s# Value [\'%s\'] ? ' % (
                hint, (value is None and '<none>' or value),))
            sys.stderr.flush()
            ...
            if line:
                if line[:2] == '0x':
                    if line[:4] == '0x0x':
                        ...
                        value = syntax.clone(hexValue=line[2:])
            ...
            if isinstance(node, MibTable):
                ...
                val = getValue(
                    ...
                    verboseFlag and '# Scalar %s::%s (type %s)\r\n' % (
                        mibName, symName, node.syntax.__class__.__name__)
                    or '')
